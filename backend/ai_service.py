...
【负面活动识别与属性扣除】非常重要，必须仔细判断！

识别为负面活动的情况（需要扣除属性）：
1. **沉迷娱乐**：打游戏一上午/一下午/几小时、刷抖音/短视频、追剧停不下来、看小说停不下来
   → 扣除 willpower(意志) -1~2，因为缺乏自控力
   → 如果时间很长（3小时+），扣除更多

2. **熬夜/作息不规律**：熬夜到凌晨、通宵、晚睡
   → 扣除 willpower(意志) -1，strength(力量) -1

3. **不健康饮食**：吃垃圾食品、暴饮暴食、吃太多零食
   → 扣除 strength(力量) -1

4. **拖延/偷懒**：拖延任务、偷懒、无所事事一整天
   → 扣除 willpower(意志) -1~2

5. **过度社交/水群**：水群几小时、无意义社交
   → 扣除 willpower(意志) -1

负面活动的特征：
- exp_gained 应该很少（5-15）或为0
- gold_gained 应该很少（1-5）或为0
- attribute_changes 中对应属性应为**负值**
- comment 应该吐槽但带有鼓励改进的意味

示例：
- "打了一上午游戏" → exp:5, gold:2, attribute_changes:{"willpower": -2}, comment:"好家伙，一上午就献给游戏了？你的意志力正在疯狂掉血啊！建议下次设个闹钟，2小时就停"
- "熬夜到3点" → exp:3, gold:1, attribute_changes:{"willpower": -1, "strength": -1}, comment:"熬夜冠军就是你！不过你的力量和意志都在默默流泪..."
- "刷了一下午抖音" → exp:5, gold:2, attribute_changes:{"willpower": -1}, comment:"短视频的魔力真大啊，你的意志力已经被吞噬了"

注意：负面活动也要返回 JSON，只是数值较低且属性为负！
